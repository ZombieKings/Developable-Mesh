...
template <typename DerivedV, typename DerivedF, typename DerivedL>
void Zombie::cal_squard_edge_lengths(const Eigen::MatrixBase<DerivedV>& V,
	const Eigen::MatrixBase<DerivedF>& F,
	Eigen::PlainObjectBase<DerivedL>& L)
{
	typedef typename DerivedF::Index Index;
	...
	case 2: {
		...
		L.resize(Enum, 1);
		for (int i = 0; i < Enum; ++i)
			L(i) = (V.col(F(1, i)) - V.col(F(0, i))).squaredNorm();
		break;
	}
	case 3: {
		assert(V.rows() == 3 && "This function is only for triangle mesh!");
		...
		{
			const auto& fv = F.col(f);
			L(0, f) = (V.col(fv[1]) - V.col(fv[2])).squaredNorm();
			L(1, f) = (V.col(fv[2]) - V.col(fv[0])).squaredNorm();
			L(2, f) = (V.col(fv[0]) - V.col(fv[1])).squaredNorm();
		}
		break;
	}
	}

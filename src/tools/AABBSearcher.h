...
public:
    ...
    void build(const Eigen::MatrixBase<Derived1>& vertices,
               const Eigen::MatrixBase<Derived2>& faces ){
        /// Bake triangle set
        const int nfs = faces.cols();
        triangles.reserve(nfs * 3);
    ...
    void build(const Mesh& mesh ){
        /// Bake triangle set
        triangles.reserve(mesh.n_faces() * 3);
        for(const auto  &f : mesh.faces()){
            auto fv = mesh.vertices(f);
            Point_3 v0 = tr(mesh.position(*fv));
            Point_3 v1 = tr(mesh.position(*(++fv)));
            Point_3 v2 = tr(mesh.position(*(++fv)));
            triangles.push_back(Triangle(v0,v1,v2));
        }
        // constructs AABB tree
        tree.rebuild(triangles.begin(),triangles.end());
    ...
    template <typename Derived1, typename Derived2>
    void closest_point( const Eigen::MatrixBase<Derived1>& queries,
                        Eigen::MatrixBase<Derived2>& footpoints ) const{
    ...
    Eigen::Matrix<Derived, 3, 1> closest_point(const Eigen::Matrix<Derived, 3, 1> &query)  const{
    ...
    void closest_point( const Eigen::MatrixBase<Derived1>& queries,
                        ...
                        Eigen::MatrixBase<Derived3>& indexes )  const{
        for(int iq=0; iq<queries.cols(); ++iq){
            Point_3 query = tr(queries.col(iq));
    ...
    void closest_point( const Vertex& query, Vertex& footpoint,
                        typename FaceMatrix::Scalar& index )  const{
        Point_3 q= tr(query);
        Point_and_primitive_id pp = tree.closest_point_and_primitive(q);
        CIterator id = pp.second;
        std::size_t index_in_vector = std::distance(triangles.begin(),id);
        assert( triangles[index_in_vector] == *id );
        footpoint = tr(pp.first);
    ...
    template <typename Derived1, typename Derived2, typename Derived3>
    void barycentric( const Eigen::MatrixBase<Derived1>& footpoints,
                      const Eigen::MatrixBase<Derived2>& indexes,
                      Eigen::MatrixBase<Derived3>& coordinates )  const{

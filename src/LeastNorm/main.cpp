...
int main(int argc, char** argv)
{
	...
	epsilon = std::max(inter_p_.size() * pow(10, -8), pow(10, -5));
...
double cal_error(const Eigen::Matrix3Xf& V, const Eigen::Matrix3Xi& F, const Eigen::Matrix3Xf& A, int flag)
{
	...
	if (flag)
	{
		...
		{
			max = abs(2.0 * M_PI - angle_sum_(inter_p_[i])) > max ? abs(2.0 * M_PI - angle_sum_(inter_p_[i])) : max;
		}
		return max;
	}
	else
	{
		...
		for (size_t i = 0; i < inter_p_.size(); ++i)
		{
			averange += angle_sum_(inter_p_[i]);
		}
...
void cal_laplace(const Eigen::Matrix3Xf& V, const Eigen::Matrix3Xi& F, const Eigen::Matrix3Xf& A, Eigen::SparseMatrix<float>& L, Eigen::MatrixX3f& b)
{
	...
	for (size_t i = 0; i < boundary_p_.size(); ++i)
	{
		triple.push_back(Eigen::Triplet<float>(boundary_p_[i], boundary_p_[i], 100));
	}
	...
	float sum_area = areas.sum() / float(inter_p_.size());

	for (int r = 0; r < inter_p_.size(); ++r)
	{
		L.row(inter_p_[r]) *= sum_area / (2.0f * areas(inter_p_[r]));
	}
	...
	//固定边界
	for (size_t ib = 0; ib < boundary_p_.size(); ++ib)
	{
		b.row(boundary_p_[ib]) = V.col(boundary_p_[ib]).transpose() * 100;
	}
	//变形目标
...
void cal_least_norm(const Eigen::Matrix3Xf& V, const Eigen::Matrix3Xi& F, const Eigen::Matrix3Xf& A, Eigen::SparseMatrix<float>& N, Eigen::VectorXf& b)
{
	...
	b.resize(inter_p_.size() + boundary_p_.size() * 3);
	b.setZero();
	for (size_t i = 0; i < inter_p_.size(); ++i)
	{
		b(i) = 2.0f * M_PI - sum_angle(inter_p_[i]);
	}
	...
	for (size_t i = 0; i < boundary_p_.size(); ++i)
	{
		...
		{
			triple.push_back(Eigen::Triplet<float>(inter_p_.size() + i * 3 + j, boundary_p_[i] * 3 + j, 1));
			b(inter_p_.size() + i * 3 + j) = 0;
		}
	}
	N.resize(inter_p_.size() + boundary_p_.size() * 3, V.cols() * 3);

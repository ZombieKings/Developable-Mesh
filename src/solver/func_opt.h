...
namespace func_opt {
	...
	protected:
		...
		Eigen::VectorXi degrees_;
		std::vector<std::vector<size_t>> indices_;
		Eigen::SparseMatrix<bool> V2V_;
		...
		Eigen::SparseMatrix<DataType> GtG_;
		Eigen::SparseMatrix<DataType> Gau_;

		surface_mesh::Surface_mesh topology_;
		std::vector<double*> nch_ptr_cache_, ch_ptr_cache_;

	protected:
		void mesh2matrix(const surface_mesh::Surface_mesh& mesh, MatrixType& V, Eigen::Matrix3Xi& F);
		void cal_topo(const Eigen::Matrix3Xi& F, int Vnum, Eigen::SparseMatrix<DataType>& F2V, Eigen::SparseMatrix<bool>& V2V);

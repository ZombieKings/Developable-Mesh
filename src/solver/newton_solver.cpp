...
namespace opt_solver {
	...
	int newton_solver::solve_sqp(double* x)
	{
		...
		for (int i = 0; i < iter_num; ++i)
		{
			...
			{
				// Cauchy point is inside, evaluate Newton point
				//compute_D(&D[0]);
				compute_D(D.data());
				D *= mu;
				//add_to_diag(&D[0]);
				...
				if (slv_.info() == Eigen::Success)
				{
					...
					if ((step_).norm() < radius)
					{ // Newton point is inside of the radius
						step_type = 1;
						std::cerr << "\tNP";
						at_border = false;
					}
			...
			if (ratio < 0 && step_type == 0)
			{ // accept bad step for NP and DL
				x0 = ori_x;
			}
